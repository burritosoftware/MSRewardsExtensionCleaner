...
echo.
if /I NOT "%serverversion%"=="%localversion%" (goto patch) else (echo No updates were found, log finished at %date%-%time% >> "%~dp0resources\log.log" && color 47 && echo Looks like there's no updates for MS Rewards. Not cleaning. && goto endscreen)
...
echo.
color 20
echo Cleaning finished, log finished at %date%-%time% >> "%~dp0resources\log.log"
